...
const uint32_t lzma_crc32_table_le[8][256] = {
    {
...
uint32_t lzma_crc32_le(const uint8_t *buf, size_t size, uint32_t crc);
...
uint32_t lzma_crc32_le(const uint8_t *buf, size_t size, uint32_t crc)
{
    ...
    if (size > 8) {
        ...
        while ((uintptr_t)(buf) & 7) {
            crc = lzma_crc32_table_le[0][*buf++ ^ A(crc)] ^ S8(crc);
            --size;
        }
        ...
        while (buf < limit) {
            ...
            crc = lzma_crc32_table_le[7][A(crc)]
                ^ lzma_crc32_table_le[6][B(crc)]
                ^ lzma_crc32_table_le[5][C(crc)]
                ^ lzma_crc32_table_le[4][D(crc)];
            ...
            crc = lzma_crc32_table_le[3][A(tmp)]
                ^ lzma_crc32_table_le[2][B(tmp)]
                ^ crc
                ^ lzma_crc32_table_le[1][C(tmp)]
                ^ lzma_crc32_table_le[0][D(tmp)];
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    printf("\n");
    printf("         De Blob 2 Checksum Resigner tool by Fredric Baeckstrom  \n");
    printf("  ======================================================= version %s ===\n\n", VER);


    // Check the number of arguments supplied, if not enough show usage to the user.
    if (argc <= 2) {
        printf("Usage: %s <savefile> <output>\n", argv[0]);
        return(-1);
    }

    // Store input and output filenames.
    char* file_input = argv[1];              // Get input filename
    char* file_output = argv[2];             // Get output filename
    ...
    // Remove current checksum and replace it with zero's as this is how the game calculates the new checksum.
    buffer[8] = 0x00;
    buffer[9] = 0x00;
    buffer[10] = 0x00;
    buffer[11] = 0x00;

    uint32_t new_checksum = lzma_crc32_le((const unsigned char*)buffer, buffer_size, 0xFFFFFFFF);
    printf("- New Checksum: 0x%08x  (%u)\n", new_checksum, new_checksum);

    buffer[8] = ((new_checksum >> 24) & 0xff);
    buffer[9] = ((new_checksum >> 16) & 0xff);
    buffer[10] = ((new_checksum >> 8) & 0xff);
    buffer[11] = (new_checksum & 0xff);
